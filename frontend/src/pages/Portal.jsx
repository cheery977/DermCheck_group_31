...
export default function Portal() {
  ...
  // Stats only need to load once — they're global counts, not filter-dependent
  useEffect(() => {
    axios.get('/api/portal/stats').then((r) => setStats(r.data))
  }, [])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    axios
      .get('/api/portal/cases', { params, signal: abortRef.current.signal })
  ...
  const handleReview = async () => {
    ...
    setSaving(false)
    setSelected(null)
    // Refresh both list and stats after a review
    const params = filter !== 'all' ? { status: filter } : {}
            ...
            <tbody>
              {cases.map((c) => (
                <tr key={c.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-600">{Math.round(c.confidence * 100)}%</td>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="p-6 space-y-4">
              ...
              <div className="text-sm text-slate-600 space-y-1">
                <p><span className="font-medium">AI confidence:</span> {Math.round(selected.confidence * 100)}%</p>
              ...
              <div className="flex justify-between gap-3">
                ...
                <button onClick={handleReview} disabled={saving} className="btn-primary flex-1">
